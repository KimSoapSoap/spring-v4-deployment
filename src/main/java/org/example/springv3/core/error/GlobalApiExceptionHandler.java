...
public class GlobalApiExceptionHandler {
    ...
    public ResponseEntity<?> ex401(Exception e) {
        log.warn("{ssar}님 {ip, device}에서 인증 실패 : "+e.getMessage());
    ...
    public ResponseEntity<?> ex403(Exception e) {
        log.warn("{ssar}님 {ip, device}에서 권한 실패 : "+e.getMessage());
    ...
    public ResponseEntity<?> ex404(Exception e) {
        log.info("{ssar}님 {ip, device}에서 자원 찾기 실패 : "+e.getMessage());
    ...
    public ResponseEntity<?> ex500(Exception e) {
        log.error("{ssar}님 {ip, device} : "+e.getMessage());
    ...
    public ResponseEntity<?> ex(Exception e) {
        log.error("{ssar}님 {ip, device}에서 알 수 없는 에러 : "+e.getMessage());
